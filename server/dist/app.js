...
exports.recommendMovies =
  exports.getMovieSummary =
  exports.getTrendingMovies =
    void 0;
...
function recommendMovies(type) {
  ...
  retrieveMovies()
    ...
    .then(function (trendingMovies) {
      // TODO: Rewrite/filter this block, trendingMovie function was rewritten to return movie objects not original response
      var movies = [];
      for (var i = 0; i < trendingMovies.length; i++) {
        movies.push({
          title: trendingMovies[i].movie.title,
    ...
    .then(function (trendingMovies) {
      ...
      for (var i = 0; i < trendingMovies.length; i++) {
        if (!Ids.includes(trendingMovies[i].id)) {
          console.log(
            ""
              .concat(trendingMovies[i].title, ": ")
              .concat(trendingMovies[i].year)
          );
